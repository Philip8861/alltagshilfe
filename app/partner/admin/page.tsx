...
export default async function PartnerAdminPage() {
  ...
  return (
    <article>
      ...
      <section className="mt-12 overflow-x-auto rounded-2xl border border-[#0F4F68]/10 bg-white shadow-sm" aria-label="Pflegebox-Abschlüsse">
            ...
            {orders.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-neutral-600">
                  Noch keine Abschlüsse in der Datenbank.
                </td>
              </tr>
            ) : (
              orders.map((o) => (
                <tr key={o.id} className="hover:bg-neutral-50/80">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-neutral-600">
                    {o.partner_id ? o.partner_id.slice(0, 8) + "…" : <span className="font-sans text-amber-800">nicht zugeordnet</span>}
                  </td>
        ...
        <p className="border-t border-[#0F4F68]/10 px-4 py-3 text-xs text-neutral-600">
          Zuordnung zu einem Partner in Supabase SQL:{" "}
      ...
      <div className="mt-8 rounded-2xl border border-[#0F4F68]/10 bg-[#F2F9FA]/40 p-5 text-sm text-neutral-700">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5">
          ...
          <li>
            Konfigurator-Link mit Partner-Zuordnung:{" "}
            <code className="rounded bg-white px-1">/pflegebox?partner=PARTNER_UUID</code> (UUID aus der Tabelle
            partner_profiles).
          </li>
